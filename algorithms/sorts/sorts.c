...
{
	for (int size_l = 0; size_l < n - 1; ++size_l) {
		for (int j = 0; j < n - size_l - 1; ++j) {
...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr, "merge sort execution time: %0.6f secs.\n", ELAPSE_TIME(end - start));
	...
	fprintf(stderr, "shell sort execution time: %0.6f secs.\n", ELAPSE_TIME(end - start));
