 ...
 *
 * Herbert Schildt,"Building your own C Interpreter." i Dr. Dobb's Journal of
 * Software Tools v14, (August, 1989):38 (16 pages).
 */
...
main (int argc, char *argv[]) {
  ...
  prog = p_buf;                 /* set program pointer to start of buffer */
  /* load the program to execute */
  if (!load_program (p_buf, argv[1]))
    exit (1);

  /*
   * if (setjmp(e_buf)) exit(1);                        /* initialize
   * long jump buffer
   */

  prescan ();                   /* find the location of all functions and
...
interp_block (void) {
  ...
  do {
    ...
    /*
     * If interpreting single statement, return on first
     * semicolon.
     */
    ...
    if (token_type == IDENTIFIER) {
      ...
      if (*token == '{')        /* is a block */
        block = 1;              /* interpreting block, not
                                 * statement */
      else
      ...
      case ELSE:               /* process an else statement */
        find_eob ();            /* find end of else block and
                                 * continue */
        break;
...
load_program (char *p, char *fname) {
  ...
  while ((c = getc (fp)) != EOF && i < PROG_SIZE) {
    *p = c;
     ...
     * end-of-line, so I have to add a LF after a CR. t.s.yang
     * 10/02/93 UCB
     */
    if (c == '\r') {
      *p = '\n';
  ...
  do {
    ...
    if (tok == CHAR || tok == INT) {    /* is global var */
      ...
      if (*token == '(') {      /* must be assume a function */
        ...
        prog++;
        /*
         * prog points to opening curly brace of
         * function
         */
      } else
        putback ();
...
/*
 * Return the entry point of the specified function. Return NULL if not
 * found.
 */
char *
find_func (char *name) {
...
call (void) {
  ...
  if (loc == NULL)
    ...
    get_params ();              /* load the function's parameters with the
...
get_params (void) {
  ...
  do {                          /* process comma-separated list of parameters */
    ...
    if (*token != ')') {
      ...
      get_token ();
      /*
       * link parameter name with argument already on local
       * var stack
       */
...
void
assign_var (char *var_name, int value) {
  register int i;
...
/*
 * Determine if an identifier is a variable. Return 1 if variable is found; 0
 * otherwise.
 */
int
is_var (char *s) {
...
exec_if (void) {
  ...
  if (cond) {                   /* is true so process target of IF */
    interp_block ();
  } else {                      /* otherwise skip around IF block and process
                                 * the ELSE, if present */
    find_eob ();                /* find start of next line */
    get_token ();
    if (tok != ELSE) {
      putback ();               /* restore token if no ELSE is
                                 * present */
      return;
    }
    interp_block ();
