...
UpdateDisplay (short int f, short int t, short int redraw, short int isspec)
{
  ...
  if (redraw)
      ...
      while (i < 19)
	{
      ...
      gotoXY (3, 21);
      if (flag.reverse)
	printz ("  h    g    f    e    d    c    b    a");
      else
	printz ("  a    b    c    d    e    f    g    h");
      for (sq = 0; sq < 64; sq++)
	DrawPiece (sq);
    }
  else
...

{
  ...
  while (!(ok || flag.quit))
      ...
      else if (strcmp (s, "white") == 0)
	{
	  ...
	  flag.force = false;
	  Sdepth = 0;
	  /*ok = true;/* dont start automatically on white command */
	}
