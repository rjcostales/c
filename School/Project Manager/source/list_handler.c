...
int
list_insert_name (list * head, char *name) {
  list_node *temp;
  int i;
...
int
list_insert_text (list * head, char *name, char *text) {
  list_node *temp;
  int i;
  ...
  if ((*head)
      && ((*head)->text = (char *) malloc ((size_t) (strlen (text) + 1)))) {
    strcpy ((*head)->name, name);
...
char *
list_get_text (list head, char *name) {
  while (head) {
...
void
list_delete (list * head, char *name) {
  list_node *temp;
...
list
list_locate (list head, char *name) {
  if (head) {
    if (strcmp (name, head->name) == 0)
      return head;
    else
...
int
list_index (list head, char *name) {
  int i;
...
int
obj_type_insert (ot_tree * root, char *type) {
  int i;
...
ot_tree
obj_type_locate (ot_tree root, char *type) {
  int i;
...
int
rel_def_insert (rd_tree * root, char *domain, char *relation,
                char *range, char *inverse) {
  int i;
    ...
    if (!(i = strcmp (domain, (*root)->domain)))
      if (!(i = strcmp (relation, (*root)->relation)))
        if (!(i = strcmp (range, (*root)->range)))
          return FALSE;
    if (i < 0)
      return rel_def_insert (&((*root)->left), domain, relation,
                             range, inverse);

    if (i > 0)
      return rel_def_insert (&((*root)->right), domain, relation,
                             range, inverse);
  } else {
...
rd_tree
rel_def_locate (rd_tree root, char *domain, char *relation, char *range) {
  int i;
