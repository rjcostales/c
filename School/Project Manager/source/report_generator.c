...
void
obj_type_count_print (ot_tree root) {
  if (root) {
    obj_type_count_print (root->left);

    fprintf (output, " %s - %d\n",
             root->type, obj_count_type (Object_Table, root->type));
...
void
rel_print_def (rd_tree root) {
  if (root) {
    if (root->left)
      rel_print_def (root->left);

    if (!strcmp (root->relation, root->inverse)
        && !strcmp (root->domain, root->range))
      fprintf (output, "%s %s;\n", root->domain, root->relation);

    else if (strcmp (root->relation, root->inverse) < 0)
      fprintf (output, "%s %s/%s %s;\n", root->domain, root->relation,
               root->inverse, root->range);
...
void
rel_print_level (char *name, int undecl, int level, int max) {
...
void
report_relations (char *filename) {
  int i = 0;
  ...
  while (Relation_Table[i]) {
    fprintf (output, "%s\t%s", Relation_Table[i]->subject,
             Relation_Table[i]->relation);
...
void
report_obj_by_name (char *filename) {
  int i = 0;
...
void
report_obj_by_type (char *filename) {
  int i = 0;
