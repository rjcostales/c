...
int
obj_insert (object * table, obj_name name, obj_type type) {
  int i = 0;
...
int
obj_count_type (object * table, char *type) {
  int i = 0, count = 0;
...
void
obj_swap (object * table, int i, int j) {
  object temp;
...
void
obj_sort_name (object * table, int left, int right) {
  int i, last;
...
void
obj_sort_type (object * table, int left, int right) {
  int i, last, c;
  ...
  for (i = left + 1; i <= right; i++)
    if ((c = strcmp (table[i]->type, table[left]->type)) < 0)
      obj_swap (table, ++last, i);
             ...
             && (strcmp (table[i]->name, table[left]->name) < 0))
      obj_swap (table, ++last, i);
...
int
rel_insert (relationship * table, obj_name subject, rel_name relation) {
  int i = 0;

  while (table[i]) {
    if ((!strcmp (table[i]->subject, subject))
        && (!strcmp (table[i]->relation, relation)))
      return i;
    else
      i++;
  }
...
void
rel_swap (relationship * table, int i, int j) {
  long temp;
...
void
rel_sort_relation (relationship * table, int left, int right) {
  int i, last, c;
  ...
  for (i = left + 1; i <= right; i++)
    if ((c = strcmp (table[i]->relation, table[left]->relation)) < 0)
      rel_swap (table, ++last, i);
             ...
             && (strcmp (table[i]->subject, table[left]->subject) < 0))
      rel_swap (table, ++last, i);
...
void
rel_sort_object (relationship * table, int left, int right) {
  int i, last, c;
  ...
  for (i = left + 1; i <= right; i++)
    if ((c = strcmp (table[i]->subject, table[left]->subject)) < 0)
      rel_swap (table, ++last, i);
             ...
             && (strcmp (table[i]->relation, table[left]->relation) < 0))
      rel_swap (table, ++last, i);
...
void
rel_dispose (relationship * table) {
  int i = 0;
...
int
att_search (object * table, att_name name) {
  int i = 0;
