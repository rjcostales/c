...
int
is_rel_name (char *object_type, char *relation_name) {
  obj_type_node *temp;
...
int
is_att_name (char *object_type, char *attribute_name) {
  obj_type_node *temp;
...
int
is_global_att_name (char *attribute_name) {
...
int
is_global_rel_name (char *relation_name) {
...
int
is_rel_def (char *domain, char *relation, char *range) {
...
char *
find_inverse (char *domain, char *relation, char *range) {
  rel_def_node *temp;

  temp =
    (rel_def_node *) rel_def_locate (relation_defs, domain, relation, range);
  return temp->inverse;
}

int
add_obj_type (char *object_type) {
...
int
add_rel_name (char *object_type, char *relation_name) {
  obj_type_node *temp;
...
int
add_att_name (char *object_type, char *attribute_name) {
  obj_type_node *temp;
  ...
  if (is_att_name (object_type, attribute_name)) {
    printf
      (" DEFINITION ERROR - ATTRIBUTE NAME %s FOR %s ALREADY DEFINED\n",
...
int
add_rel_def (char *domain, char *relation, char *range, char *inverse) {
...
int
add_object (obj_name name, obj_type type) {
  if (!is_obj_type (type)) {
...
int
add_attribute (obj_name name, att_name attribute, char *text) {
  int i;
...
int
add_relation (obj_name name, rel_name relation, obj_name obj_of_rel) {
  int i;
  ...
  while (Relation_Table[i]) {
    ...
    while (curr) {
      if (range = find_type (curr->name)) {
        ...
        status &=
          add_relation (curr->name, relation, Relation_Table[i]->subject);
      } else {
...
/*				list_delete(&(Relation_Table[i]->obj_list), curr->name); */
  ...
  for (i = 0; i < size_obj; i++) {
    for (j = 0; j < size_rel; j++) {
      if (strcmp (Relation_Table[j]->subject, Object_Table[i]->name))
        continue;
      if (!strcmp (Relation_Table[j]->relation, "called_by")
          || !strcmp (Relation_Table[j]->relation, "used_by"))
        break;
    }
    if (j >= size_rel) {
  ...
  printf
    ("\n %d objects\n\n %d relations\n\n %d undeclared\n\n %d unused globals\n\n %d unused subroutines\n",
     obj_size (Object_Table), rel_size (Relation_Table),
     list_length (undeclared), list_length (unused_globals),
