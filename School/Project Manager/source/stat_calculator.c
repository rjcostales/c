...
void
matrix_set (int i, int j, matrix m, int s) {
  m[(i * s) + j] |= 1;
}

void
matrix_copy (matrix m, matrix n, int s) {
  int i;
...
void
matrix_product (matrix m, matrix n, matrix o, int s) {
  int i, j, k;
...
void
compute_levels (obj_type type, rel_name relation) {
  int i, s = 1;
  ...
  for (i = 0; Relation_Table[i]; i++) {
    if (!strcmp (type, find_type (Relation_Table[i]->subject)) &&
      ...
      printf (" %s %s contains %d %s", type,
              Relation_Table[i]->subject, length =
              list_length (temp), relation);
      ...
      while (temp) {
        matrix_set (list_index (process_list, temp->name),
                    list_index (process_list,
                                Relation_Table[i]->subject), init, s);
        temp = temp->next;
      }
    }
  }
...
void
compute_calls (obj_type type, rel_name relation) {
  int i;
  list temp;
  ...
  for (i = 0; Relation_Table[i]; i++) {
    if (!strcmp (type, find_type (Relation_Table[i]->subject)) &&
      ...
      printf (" %s %s %s %d modules", type,
              Relation_Table[i]->subject, relation, length =
              list_length (temp));
...
void
compute_complexity (obj_type type, att_name path) {
  int i, line_count, region_count, total_line_count =
    0, total_region_count = 0;
  char *filename;
