...
int
ModTree::findInt (Node * node) {
  while (prevInt (++(node->mO), node));
  return node->mO;
}

int
ModTree::prevInt (int i, Node * node) {
  if (node->mX == i)
    return 1;
...
void
ModTree::save (char *filename)  // a crude function, but it works!
{
