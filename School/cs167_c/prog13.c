...
void
print_list (node * list, char *title) {
  fprintf (output, "%s\n", title);
  while (list != NULL) {
    fprintf (output, "%d\n", list->value);
    list = list->next;
...
void
print_reverse (node * list, char *title) {      /* intermediate call to print title */
  fprintf (output, "%s\n", title);      /* call recursive function */
  reverse_print (list);
}

void
reverse_print (node * list) {
  if (list->next != NULL)
