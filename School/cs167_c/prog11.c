...
void
sort (char **list, int size) {  /* list is ragged array of strings of size */
  int i, j;
  char *temp;
  for (i = 0; i < size; ++i)
    for (j = i + 1; j < size; ++j)
...
void
print (char **list, int size, char *title) {    /* print ragged array with a
                                                 * title */
  int i;
  puts (title);
  for (i = 0; i < size; i++)
...
void
code_list (char **list, int size, char m) {     /* encode the words in list
                                                 * if m == 'e' otherwise
                                                 * decode */
  int i;
  for (i = 0; i < size; i++)
...
void
encode (char *word) {           /* called by code_list */
  int i;
  for (i = 0; word[i]; i++)
    if (isalpha (word[i]))
      if (islower (word[i]))
...
void
decode (char *word) {           /* called by code_list */
  int i;
  for (i = 0; word[i]; i++)
    if (isalpha (word[i]))
      if (islower (word[i]))
...
void
print_error () {                /* print usage errror */
