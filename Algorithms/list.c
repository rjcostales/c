...
void insert(list * head, char *name)
{
    node   *temp;
...
void append(list * head, char *name)
{
    if (*head)
...
node   *locate(list head, char *name)
{
    if (head) {
        if (strcmp(name, head->name) == 0)
            return head;
...
{
    if (head) {
        printf("%s\t\"%s\"%s", head->name, head->text, (head->next) ? ";\n" : ".\n");
